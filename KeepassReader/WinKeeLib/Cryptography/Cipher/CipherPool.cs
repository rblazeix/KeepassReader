// --------------------------------------------------------------------------------------------------------------------
// <copyright file="CipherPool.cs" company="">
//   
// </copyright>
// <summary>
...
namespace KeePassLib.Cryptography.Cipher
{
    ...
    public sealed class CipherPool
    {
        ...
        public ICipherEngine this[int nIndex]
        {
            get
            {
                if ((nIndex < 0) || (nIndex >= this.m_vCiphers.Count))
                {
                    throw new ArgumentOutOfRangeException("nIndex");
                }

                return this.m_vCiphers[nIndex];
            }
        }
        ...
        /// </summary>
        /// <param name="csEngine">
        /// Cipher engine to add. Must not be <c>null</c>.
        /// </param>
        public void AddCipher(ICipherEngine csEngine)
        {
            ...
            for (int i = 0; i < this.m_vCiphers.Count; ++i)
            {
                if (this.m_vCiphers[i].CipherUuid.Equals(csEngine.CipherUuid))
                {
                    return;
                }
            }

            this.m_vCiphers.Add(csEngine);
        }
        ...
        /// </param>
        /// <returns>
        /// Index of the requested cipher. Returns <c>-1</c> if
        ...
        public int GetCipherIndex(PwUuid uuidCipher)
        {
            for (int i = 0; i < this.m_vCiphers.Count; ++i)
            {
                if (this.m_vCiphers[i].CipherUuid.Equals(uuidCipher))
                {
                    return i;
                }
            }
        ...
        /// </param>
        /// <returns>
        /// Cipher with the specified name or <c>-1</c> if
        ...
        public int GetCipherIndex(string strDisplayName)
        {
            for (int i = 0; i < this.m_vCiphers.Count; ++i)
            {
                if (this.m_vCiphers[i].DisplayName == strDisplayName)
                {
                    return i;
                }
            }
